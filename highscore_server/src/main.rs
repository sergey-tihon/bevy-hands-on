...
impl HighScoreTable {
    fn new() -> Self {
        if std::path::Path::new("high_score.json").exists() {
            let file = std::fs::File::open("high_score.json").unwrap();
    ...
    fn add_entry(&mut self, entry: HighScoreEntry) {
        ...
        self.entries.truncate(10); // Keep only top 10 scores
        self.save();
    }

    fn save(&self) {
        let file = std::fs::File::create("high_score.json").unwrap();
